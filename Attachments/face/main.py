...
class Backend:
    def __init__(self):
        self.gender_net = self._load_model(C.GENDER_PROTO, C.GENDER_MODEL, "Gender detection")
        self.face_net = self._load_model(C.FACE_PROTO, C.FACE_MODEL, "Face detection")

    def _load_model(self, proto_path, model_path, model_name):
    ...
    def load_labels(self):
        try:
            ...
            with open(C.LABELS_PATH, "rb") as f:
    ...
    def detect_single_face_for_training(self, img):
        if self.face_net is None: return None
        (h, w) = img.shape[:2]
        ...
        if highest_confidence > C.DNN_CONFIDENCE:
            i = best_detection_idx
            box = detections[0, 0, i, 3:7] * np.array([w, h, w, h])
            (startX, startY, endX, endY) = box.astype("int")
            face = img[startY:endY, startX:endX]
            if face.size == 0: return None
            gray_face = cv2.cvtColor(face, cv2.COLOR_BGR2GRAY)
            return cv2.resize(gray_face, C.TRAINING_IMG_SIZE)
        return None
    ...
    def train_recognizer(self):
        ...
        with open(C.ID_MAP_PATH, "wb") as f:
    ...
    def detect_gender(self, face_img):
        ...
        try:
            ...
            return C.GENDER_LIST[gender_preds[0].argmax()]
...
class ManageTab(QWidget):
    ...
    def init_ui(self):
        ...
        right_group = QGroupBox("Registered Users");
        right_layout = QVBoxLayout(right_group)
        self.user_list = QListWidget();
        right_layout.addWidget(self.user_list)
        layout.addWidget(left_pane, 2);
        ...
        Animator.apply_push_animation(self.select_img_btn);
        Animator.apply_push_animation(self.add_btn)
        Animator.apply_push_animation(self.update_btn);
        Animator.apply_push_animation(self.delete_btn)
    ...
    def add_face(self):
        if self.backend.face_net is None: QMessageBox.critical(self, "Error",
                                                               "Face detection model not loaded."); return
        ...
        self.status_updated.emit(f"Added face for {username}.");
        QMessageBox.information(self, "Success", f"Face added for user '{username}'.")
    ...
    def update_face(self):
        ...
        if QMessageBox.question(self, "Confirm Update",
                                f"This will replace all images for '{username}'. Continue?") == QMessageBox.StandardButton.Yes:
            img = cv2.imread(self.img_path);
            face = self.backend.detect_single_face_for_training(img)
            ...
            save_path = os.path.join(user_dir, "1.png");
            cv2.imwrite(save_path, face)

            labels_dict = self.backend.load_labels();
            labels_dict[username] = [face]
            self.backend.save_labels(labels_dict)

            self.status_updated.emit(f"Updated face for {username}.");
            QMessageBox.information(self, "Success", f"Face for '{username}' updated.")
            self.model_needs_training.emit()
    ...
    def delete_face(self):
        ...
        if QMessageBox.question(self, "Confirm Delete",
                                f"Delete all data for '{username}'?") == QMessageBox.StandardButton.Yes:
            labels_dict = self.backend.load_labels()
            if username in labels_dict:
                shutil.rmtree(os.path.join(C.FACES_DIR, username), ignore_errors=True);
                del labels_dict[username]
                self.backend.save_labels(labels_dict)
                self.status_updated.emit(f"Deleted user: {username}.");
    ...
    def refresh_user_list(self):
        ...
        users = sorted(self.backend.load_labels().keys());
        self.user_list.addItems(users)
        self.user_list_changed.emit(len(users))

    def clear_inputs(self):
        self.user_entry.clear(); self.img_preview.clear(); self.img_preview.setText(
            "Image Preview"); self.img_path = None
...
class RecognizeTab(QWidget):
    ...
    def init_ui(self):
        ...
        self.start_stop_btn = QPushButton("Start Recognition");
        self.start_stop_btn.clicked.connect(self.toggle_camera)
        self.update_start_stop_button(False);
    ...
    def start_camera(self):
        if self.backend.face_net is None: QMessageBox.critical(self, "Error",
                                                               "Face detection model not loaded."); return
        if not os.path.exists(C.TRAINER_PATH): QMessageBox.warning(self, "Model Not Found",
                                                                   "Please train the model first."); return
        try:
            self.recognizer = cv2.face.LBPHFaceRecognizer_create();
            self.recognizer.read(C.TRAINER_PATH)
            with open(C.ID_MAP_PATH, "rb") as f:
                self.id_to_label = pickle.load(f)
        except Exception as e:
            QMessageBox.critical(self, "Error", f"Failed to load model files: {e}"); return
        self.cap = cv2.VideoCapture(0)
        if not self.cap.isOpened(): QMessageBox.critical(self, "Camera Error", "Could not access the camera."); return
        self.camera_active = True;
        self.timer.start(15);
        self.status_updated.emit("▶️ Recognition started...")
    ...
    def stop_camera(self):
        ...
        self.status_updated.emit("⏹️ Recognition stopped.");
        self.update_start_stop_button(False)

    def update_frame(self):
        if not self.camera_active or not self.cap or self.backend.face_net is None: return
        ret, frame = self.cap.read()
        if not ret: return
        frame = cv2.flip(frame, 1)
        (h, w) = frame.shape[:2]
        blob = cv2.dnn.blobFromImage(cv2.resize(frame, (300, 300)), 1.0, (300, 300), (104.0, 177.0, 123.0))
        self.backend.face_net.setInput(blob)
        detections = self.backend.face_net.forward()

        for i in range(detections.shape[2]):
            confidence = detections[0, 0, i, 2]
            if confidence > C.DNN_CONFIDENCE:
                box = detections[0, 0, i, 3:7] * np.array([w, h, w, h])
                (startX, startY, endX, endY) = box.astype("int")
                ...
                face_roi_color = frame[startY:endY, startX:endX]
                if face_roi_color.size == 0: continue

                face_roi_gray = cv2.cvtColor(face_roi_color, cv2.COLOR_BGR2GRAY)
                resized_face = cv2.resize(face_roi_gray, C.TRAINING_IMG_SIZE)

                label_id, conf = self.recognizer.predict(resized_face)

                name = "Unknown"
                conf_text = ""
                if conf < C.RECOGNITION_CONFIDENCE:
                    name = self.id_to_label.get(label_id, "Unknown")
                    conf_text = f" ({round(conf, 1)})"

                gender = self.backend.detect_gender(face_roi_color)
                text = f"{name}{conf_text} | {gender}" if gender else f"{name}{conf_text}"
                color = (0, 255, 0) if name != "Unknown" else (0, 0, 255)

                cv2.rectangle(frame, (startX, startY), (endX, endY), color, 2)
                cv2.putText(frame, text, (startX, startY - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.6, color, 2)
    ...
    def update_start_stop_button(self, is_active):
        icon = self.style().standardIcon(
            QStyle.StandardPixmap.SP_MediaStop if is_active else QStyle.StandardPixmap.SP_MediaPlay)
        self.start_stop_btn.setText("Stop Recognition" if is_active else "Start Recognition");
        self.start_stop_btn.setIcon(icon)
...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__();
        self.setWindowTitle("VisionAI - Secure Face Recognition");
        ...
        self.manage_tab.refresh_user_list();
        self.check_camera()

    def check_camera(self):
        cap = cv2.VideoCapture(0)
        if not cap.isOpened(): QMessageBox.warning(self, "Camera Not Found",
                                                   "No camera detected. Recognition tab will not function.")
        cap.release()

    def check_models(self):
        if self.backend.face_net is None:
            QMessageBox.critical(self, "Critical Error",
                                 "Face detection model not found in 'opencv_data/models'.\n\n"
                                 "The application cannot function without it.\n\n"
                                 "Please see constants.py for download links.")
        if self.backend.gender_net is None:
            QMessageBox.warning(self, "Model Not Found",
                                "Gender detection model files not found in 'opencv_data/models'.\n\n"
                                "The gender detection feature will be disabled.\n\n"
                                "Please see constants.py for download links.")
    ...
    def init_ui(self):
        ...
        train_layout.addWidget(self.model_status_label);
        train_layout.addStretch()
        self.train_btn = QPushButton("Train Model");
        Animator.apply_morph_animation(self.train_btn)
    ...
    def train_model(self):
        QTimer.singleShot(400, self.start_training_thread)
    ...
    def start_training_thread(self):
        ...
        self.thread = QThread();
        self.worker = Worker(self.backend.train_recognizer)
    ...
    def update_model_status(self, needs_training=False):
        if needs_training:
            self.model_status_label.setText("Model Status: 🟡 Needs Training")
        elif os.path.exists(C.TRAINER_PATH):
            self.model_status_label.setText("Model Status: ✅ Trained")
        else:
    ...
    def closeEvent(self, event):
        self.recognize_tab.stop_camera(); event.accept()
